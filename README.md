...
jobs:
  claude-response:
    ...
    steps:
      - uses: mormorbump/claude-code-action@main  # Fork with OAuth support
        with:
...
steps:
  - uses: mormorbump/claude-code-action@main
    with:
      direct_prompt: |
        Update the API documentation in README.md to reflect
        the changes made to the API endpoints in this PR.
...
This action is built on top of [`mormorbump/claude-code-base-action`](https://github.com/anthropics/claude-code-base-action).
